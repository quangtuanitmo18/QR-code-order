...
async function globalSetup() {
  ...
  if (tables.length === 0) {
    ...
    if (createTableRes.ok()) {
      ...
      console.warn('[global-setup] ⚠️  Could not create table:', createTableRes.status(), await createTableRes.text())
    }
  }
  ...
  if (dishes.length === 0) {
    ...
    const sampleDishes = [
      { name: 'E2E Burger', price: 9.99, description: 'Test burger for E2E', image: 'http://localhost:4000/static/default-dish.png', status: 'Available' },
      { name: 'E2E Fries', price: 4.99, description: 'Test fries for E2E', image: 'http://localhost:4000/static/default-dish.png', status: 'Available' },
    ]
    ...
    for (const dish of sampleDishes) {
      ...
      if (createDishRes.ok()) {
        ...
        console.warn(`[global-setup] ⚠️  Could not create dish "${dish.name}":`, createDishRes.status())
      }
    }
  } else {
