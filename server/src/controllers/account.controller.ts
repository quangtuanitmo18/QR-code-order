...
export const initOwnerAccount = async () => {
  ...
  if (accountCount === 0) {
    ...
    console.log(
      chalk.bgCyan(
        `Created owner account successfully: ${envConfig.INITIAL_EMAIL_OWNER}|${envConfig.INITIAL_PASSWORD_OWNER}`
      )
    )
  }
}
...
export const updateEmployeeAccount = async (accountId: number, body: UpdateEmployeeAccountBodyType) => {
  try {
    ...
    if (!oldAccount) {
      throw new EntityError([{ field: 'email', message: 'The account you are updating no longer exists!' }])
    }
...
export const createGuestController = async (body: CreateGuestBodyType) => {
  ...
  if (table.status === TableStatus.Hidden) {
    throw new Error(`Table ${table.number} has been hidden, please choose another table`)
  }
