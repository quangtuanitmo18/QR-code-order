...
// ─── V1: Staged Executor (backward compat) ───────────────────────────────────

/**
 * V1 executor: flat stages, no state forwarding.
...
export async function executeTasks(
  ...
  for (const stage of executionPlan) {
    const stageTasks = stage.taskIds.map((id) => taskMap.get(id)!).filter(Boolean)

    if (stage.mode === 'parallel' && stageTasks.length > 1) {
      const promises = stageTasks.map((task) => executeOneTask(task, context, stateCtx))
      const settled = await Promise.allSettled(promises)
      ...
      for (let i = 0; i < settled.length; i++) {
        ...
        if (result.status === 'fulfilled') {
          results.push(result.value)
          if (result.value.status === 'completed') {
            stateCtx.taskResults.set(stageTasks[i].id, result.value.data)
          }
        } else {
          results.push({
            taskId: stageTasks[i].id,
            intent: stageTasks[i].intent,
            status: 'failed',
            reason: result.reason?.message || 'Unknown error',
            latencyMs: 0
          })
      ...
      for (const task of stageTasks) {
        try {
          const result = await executeOneTask(task, context, stateCtx)
          results.push(result)
          if (result.status === 'completed') {
            stateCtx.taskResults.set(task.id, result.data)
          }
        } catch (error) {
          results.push({
            taskId: task.id,
  ...
  log?.info(
    {
      taskCount: tasks.length,
      stageCount: executionPlan.length,
      completed: results.filter((r) => r.status === 'completed').length,
      blocked: results.filter((r) => r.status === 'blocked').length,
      failed: results.filter((r) => r.status === 'failed').length,
      deduplicated: results.filter((r) => r.status === 'deduplicated').length,
    ...
    '[Task Executor] Execution complete'
  )
